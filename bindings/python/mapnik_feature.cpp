...
void feature_add_geometries_from_wkb(feature_impl &feature, std::string wkb)
{
    geometry_utils::from_wkb(feature.paths(), wkb.c_str(), wkb.size());
}

void feature_add_geometries_from_wkt(feature_impl &feature, std::string wkt)
{
    bool result = mapnik::from_wkt(wkt, feature.paths());
    if (!result) throw std::runtime_error("Failed to parse WKT");
}

std::string feature_to_geojson(feature_impl const& feature)
{
    std::string json;
...
mapnik::value  __getitem__(feature_impl const& feature, std::string const& name)
{
    return feature.get(name);
}

mapnik::value  __getitem2__(feature_impl const& feature, std::size_t index)
{
    return feature.get(index);
}

void __setitem__(feature_impl & feature, std::string const& name, mapnik::value const& val)
{
    feature.put_new(name,val);
}

boost::python::dict attributes(feature_impl const& f)
{
...
void export_feature()
{
    ...
    class_<mapnik::feature_impl,boost::shared_ptr<mapnik::feature_impl>,
        ...
        .def("__str__",&feature_impl::to_string)
        .def("add_geometries_from_wkb", &feature_add_geometries_from_wkb)
        .def("add_geometries_from_wkt", &feature_add_geometries_from_wkt)
        .def("add_geometry", &feature_impl::add_geometry)
        .def("num_geometries",&feature_impl::num_geometries)
        ...
        .def("envelope", &feature_impl::envelope)
        .def("has_key", &feature_impl::has_key)
        .add_property("attributes",&attributes)
        .def("__setitem__",&__setitem__)
        .def("__getitem__",&__getitem__)
        .def("__getitem__",&__getitem2__)
        .def("__len__", &feature_impl::size)
        .def("context",&feature_impl::context)
